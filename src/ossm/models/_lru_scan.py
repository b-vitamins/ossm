...
def _load_extension() -> Optional[ModuleType]:
    """Compile and cache the LRU scan extension if possible."""

    global _EXTENSION, _EXTENSION_ERROR
    if _EXTENSION is not None or _EXTENSION_ERROR is not None:
        return _EXTENSION

    base = pathlib.Path(__file__).with_name("_lru_scan.cpp")
    cuda_source = base.with_name("_lru_scan_cuda.cu")
...
def try_run_lru_scan(lambda_bar: Tensor, b_seq: Tensor) -> Optional[Tensor]:
    ...
    result = extension.lru_scan(lambda_real, lambda_imag, b_paired)
