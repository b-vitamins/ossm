...
__global__ void s5_scan_kernel(
    const scalar_t* __restrict__ lambda_real,
...
at::Tensor s5_scan_cuda(const at::Tensor& lambda_real,
  ...
  AT_DISPATCH_FLOATING_TYPES(lambda_real_contig.scalar_type(), "s5_scan_cuda", [&] {
    s5_scan_kernel<scalar_t><<<blocks, threads, 0, at::cuda::getCurrentCUDAStream()>>>(
