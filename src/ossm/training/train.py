...
from ..data.datasets.collate import pad_collate
from ..models import Backbone, ClassificationHead, Head, RegressionHead, SequenceBackboneOutput
...
def step(
    ...
    values: Tensor = batch["values"].to(device)
    labels: Tensor = batch["label"].to(device)
    backbone_out: SequenceBackboneOutput = backbone(values)
