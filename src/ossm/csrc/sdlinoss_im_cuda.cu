...
struct ImTransitionBuilder {
  ...
  __device__ static Transition2x2<value_t> build(value_t a, value_t g, value_t step, scalar_t bu) {
    ...
    const value_t S = clamp_stability(value_t(1) + dt * g + dt2 * a);
...
void sdlinoss_im_forward_cuda(const at::Tensor& A,
  ...
  AT_DISPATCH_COMPLEX_TYPES(bu.scalar_type(), "sdlinoss_im_forward_cuda", [&] {
    ...
    sdlinoss_forward_tile_kernel<scalar_t, ImTransitionBuilder<scalar_t>, kSdlinossTileSteps>
