...
void dlinoss_ex_forward_cpu_kernel(const typename ComplexTraits<scalar_t>::value_t* __restrict__ a_diag,
    ...
    for (int64_t state = begin; state < end; ++state) {
      ...
      for (int64_t batch_idx = 0; batch_idx < batch; ++batch_idx) {
        ...
        for (int64_t t = 0; t < length; ++t) {
          ...
          const value_t new1_real = state0_real * m21_val + state1_real;
          const value_t new1_imag = state0_imag * m21_val + state1_imag;
...
template <typename scalar_t>
void dlinoss_ex_backward_cpu_kernel(const typename ComplexTraits<scalar_t>::value_t* __restrict__ a_diag,
                                    const typename ComplexTraits<scalar_t>::value_t* __restrict__ g_diag,
                                    const typename ComplexTraits<scalar_t>::value_t* __restrict__ step,
                                    const scalar_t* __restrict__ bu_ptr,
    ...
    for (int64_t state = begin; state < end; ++state) {
      ...
      for (int64_t batch_idx = 0; batch_idx < batch; ++batch_idx) {
        ...
        for (int64_t t = length - 1; t >= 0; --t) {
          ...
          const value_t grad_bu_real = grad_new0_real * sigma_val;
          const value_t grad_bu_imag = grad_new0_imag * sigma_val;
          *grad_bu_series = scalar_t(grad_bu_real, grad_bu_imag);
          ...
          const value_t next_state1_real = grad_new0_real * m12_val + grad_new1_real;
          const value_t next_state1_imag = grad_new0_imag * m12_val + grad_new1_imag;
...
void dlinoss_ex_forward_cpu(const at::Tensor& a_diag,
  ...
  AT_DISPATCH_COMPLEX_TYPES(bu.scalar_type(), "dlinoss_ex_forward_cpu", [&] {
    dlinoss_ex_forward_cpu_kernel<scalar_t>(a_diag.data_ptr<typename scalar_t::value_type>(),
...
torch::Tensor dlinoss_ex_forward(const at::Tensor& a_diag,
  ...
  if (bu.is_cuda()) {
#ifdef WITH_CUDA
    dlinoss_ex_forward_cuda(a_diag, g_diag, step, bu, output);
#else
    TORCH_CHECK(false, "dlinoss_ex CUDA extension was not built");
#endif
  } else {
    dlinoss_ex_forward_cpu(a_diag, g_diag, step, bu, output);
  }
...
std::vector<at::Tensor> dlinoss_ex_backward(const at::Tensor& a_diag,
  ...
  TORCH_CHECK(grad_output.dim() == 3,
              "grad_output must have shape (length, batch, ssm_size)");
  ...
  auto grad_a = at::empty_like(a_diag);
  auto grad_g = at::empty_like(g_diag);
  auto grad_step = at::empty_like(step);
  ...
  if (bu.is_cuda()) {
#ifdef WITH_CUDA
    dlinoss_ex_backward_cuda(a_diag, g_diag, step, bu, states, grad_output, grad_a, grad_g, grad_step, grad_bu);
#else
    TORCH_CHECK(false, "dlinoss_ex CUDA extension was not built");
#endif
  } else {
    AT_DISPATCH_COMPLEX_TYPES(bu.scalar_type(), "dlinoss_ex_backward_cpu", [&] {
      dlinoss_ex_backward_cpu_kernel<scalar_t>(a_diag.data_ptr<typename scalar_t::value_type>(),
                                               g_diag.data_ptr<typename scalar_t::value_type>(),
                                               step.data_ptr<typename scalar_t::value_type>(),
                                               bu.data_ptr<scalar_t>(),
