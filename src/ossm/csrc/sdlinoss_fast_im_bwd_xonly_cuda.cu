...
__global__ void im_backward_xonly_kernel(
  ...
  for (int64_t tile = num_tiles - 1; tile >= 0; --tile) {
    ...
    for (int64_t k = tile_len - 1; k >= 0; --k) {
      ...
      const scalar_t prev_x = (t > 0) ? x_only[offset_series - series] : scalar_t(0);
      const scalar_t prev_prev_x = (t > 1) ? x_only[offset_series - 2 * series] : scalar_t(0);

      const value_t x_prev_real = prev_x.real();
      const value_t x_prev_imag = prev_x.imag();
      const value_t w_prev_real =
          (t > 0) ? (prev_x.real() - prev_prev_x.real()) : value_t(0);
      const value_t w_prev_imag =
          (t > 0) ? (prev_x.imag() - prev_prev_x.imag()) : value_t(0);
...
void launch_backward_kernel_xonly(const Strides3& A_stride,
                                  ...
                                  dim3 block,
                                  cudaStream_t stream) {
  im_backward_xonly_kernel<scalar_t, VL, VB, VM, TILE><<<grid, block, 0, stream>>>(
      A,
      A_stride.sL,
      A_stride.sB,
      A_stride.sM,
