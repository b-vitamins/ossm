...
__global__ void imex2_build_tile_summaries_kernel(
  ...
  for (int64_t tile = 0; tile < num_tiles; ++tile) {
    ...
    for (int64_t k = 0; k < tile_len; ++k) {
      ...
      step_coeff_imex2<value_t>(A_val, G_val, dt, alpha, beta, gamma);
...
__global__ void imex2_prefix_tiles_per_series_kernel(
...
template <typename scalar_t, bool VL, bool VB, bool VM, int TILE>
__global__ void imex2_expand_tiles_write_states_kernel(
    const typename scalar_t::value_type* __restrict__ A,
    int64_t AsL,
    int64_t AsB,
    int64_t AsM,
...
template <typename scalar_t, bool VL, bool VB, bool VM, int TILE>
__global__ void imex2_expand_tiles_write_x_kernel(
    const typename scalar_t::value_type* __restrict__ A,
    int64_t AsL,
    int64_t AsB,
    int64_t AsM,
...
void launch_build_kernel(const Strides3& A_stride,
                         ...
                         dim3 block,
                         cudaStream_t stream) {
  imex2_build_tile_summaries_kernel<scalar_t, VL, VB, VM, TILE>
...
void launch_expand_kernel(const Strides3& A_stride,
  ...
  imex2_expand_tiles_write_states_kernel<scalar_t, VL, VB, VM, TILE>
...
void launch_expand_x_kernel(const Strides3& A_stride,
  ...
  imex2_expand_tiles_write_x_kernel<scalar_t, VL, VB, VM, TILE>
      <<<grid, block, shared_bytes, stream>>>(A,
                                   A_stride.sL,
                                   A_stride.sB,
                                   A_stride.sM,
...
void launch_prefix_kernel(const typename scalar_t::value_type* M00,
  ...
  dim3 grid(series);
  dim3 block(1);
  imex2_prefix_tiles_per_series_kernel<scalar_t, TILE>
...
void dispatch_all_xonly(int vary_mask,
  ...
  launch_prefix_kernel<scalar_t, TILE>(M00_ptr, M01_ptr, M10_ptr, M11_ptr, F0_ptr, F1_ptr, S0_w_ptr, S0_x_ptr,
                                       series, num_tiles, stream);
