...
void dlinoss_im_forward_cpu_kernel(const typename ComplexTraits<scalar_t>::value_t* __restrict__ a_diag,
  ...
  for (int64_t state = 0; state < ssm; ++state) {
    ...
    coeff22[state] = (value_t(1) + sigma * gamma) * inv_val;
  }
      ...
      for (int64_t batch_idx = 0; batch_idx < batch; ++batch_idx) {
        ...
        for (int64_t t = 0; t < length; ++t) {
          ...
          const value_t new0_real =
              state0_real * inv_val + state1_real * coeff12_val + bu_real * sigma_inv_val;
          const value_t new0_imag =
              state0_imag * inv_val + state1_imag * coeff12_val + bu_imag * sigma_inv_val;
          const value_t new1_real =
              state0_real * sigma_inv_val + state1_real * coeff22_val + bu_real * sigma2_inv_val;
          const value_t new1_imag =
              state0_imag * sigma_inv_val + state1_imag * coeff22_val + bu_imag * sigma2_inv_val;
...
template <typename scalar_t>
void dlinoss_im_backward_cpu_kernel(const typename ComplexTraits<scalar_t>::value_t* __restrict__ a_diag,
                                    const typename ComplexTraits<scalar_t>::value_t* __restrict__ g_diag,
                                    const typename ComplexTraits<scalar_t>::value_t* __restrict__ step,
                                    const scalar_t* __restrict__ bu_ptr,
  ...
  at::parallel_for(0, ssm, 1, [&](int64_t begin, int64_t end) {
    for (int64_t state = begin; state < end; ++state) {
      ...
      value_t grad_coeff12_state = value_t(0);
      value_t grad_coeff22_state = value_t(0);
      ...
      for (int64_t batch_idx = 0; batch_idx < batch; ++batch_idx) {
        ...
        for (int64_t t = length - 1; t >= 0; --t) {
          ...
          grad_coeff12_state += grad_new0_real * prev1_real + grad_new0_imag * prev1_imag;
          grad_coeff22_state += grad_new1_real * prev1_real + grad_new1_imag * prev1_imag;

          grad_sigma_inv_state += grad_new0_real * bu_real + grad_new0_imag * bu_imag;
          grad_sigma_inv_state += grad_new1_real * prev0_real + grad_new1_imag * prev0_imag;
          grad_sigma2_inv_state += grad_new1_real * bu_real + grad_new1_imag * bu_imag;
          ...
          *grad_bu_series = scalar_t(grad_bu_real, grad_bu_imag);
          ...
          const value_t next_state0_real = grad_new0_real * inv_val + grad_new1_real * sigma_inv_val;
          const value_t next_state0_imag = grad_new0_imag * inv_val + grad_new1_imag * sigma_inv_val;
          const value_t next_state1_real = grad_new0_real * coeff12_val + grad_new1_real * coeff22_val;
          const value_t next_state1_imag = grad_new0_imag * coeff12_val + grad_new1_imag * coeff22_val;
      ...
      grad_sigma_inv_state += grad_coeff12_state * (-alpha);
      grad_alpha_state += grad_coeff12_state * (-sigma_inv_val);

      const value_t t_factor = value_t(1) + sigma * gamma;
      grad_inv_state += grad_coeff22_state * t_factor;
      const value_t grad_t_state = grad_coeff22_state * inv_val;
      grad_gamma_state += grad_t_state * sigma;
      grad_sigma_state += grad_t_state * gamma;

      grad_sigma_state += grad_sigma_inv_state * inv_val;
      grad_inv_state += grad_sigma_inv_state * sigma;

      grad_sigma_state += grad_sigma2_inv_state * (value_t(2) * sigma * inv_val);
      grad_inv_state += grad_sigma2_inv_state * sigma_sq;

      const value_t inv_sq = inv_val * inv_val;
      const value_t grad_denom_state = grad_inv_state * (-inv_sq);
      grad_alpha_state += grad_denom_state * sigma_sq;
      grad_gamma_state += grad_denom_state * sigma;
      grad_sigma_state += grad_denom_state * (gamma + value_t(2) * sigma * alpha);
...
void dlinoss_im_forward_cpu(const at::Tensor& a_diag,
  ...
  AT_DISPATCH_COMPLEX_TYPES(bu.scalar_type(), "dlinoss_im_forward_cpu", [&] {
    dlinoss_im_forward_cpu_kernel<scalar_t>(a_diag.data_ptr<typename scalar_t::value_type>(),
...
void dlinoss_im_backward_cpu(const at::Tensor& a_diag,
                              const at::Tensor& g_diag,
                              const at::Tensor& step,
                              const at::Tensor& bu,
  ...
  AT_DISPATCH_COMPLEX_TYPES(bu.scalar_type(), "dlinoss_im_backward_cpu", [&] {
    dlinoss_im_backward_cpu_kernel<scalar_t>(a_diag.data_ptr<typename scalar_t::value_type>(),
                                             g_diag.data_ptr<typename scalar_t::value_type>(),
                                             step.data_ptr<typename scalar_t::value_type>(),
                                             bu.data_ptr<scalar_t>(),
...
torch::Tensor dlinoss_im_forward(const at::Tensor& a_diag,
  ...
  if (bu.is_cuda()) {
#ifdef WITH_CUDA
    dlinoss_im_forward_cuda(a_diag, g_diag, step, bu, output);
#else
    TORCH_CHECK(false, "dlinoss_im CUDA extension was not built");
#endif
  } else {
    dlinoss_im_forward_cpu(a_diag, g_diag, step, bu, output);
  }
...
std::vector<at::Tensor> dlinoss_im_backward(const at::Tensor& a_diag,
  ...
  if (bu.is_cuda()) {
#ifdef WITH_CUDA
    dlinoss_im_backward_cuda(a_diag, g_diag, step, bu, states, grad_output, grad_a, grad_g, grad_step, grad_bu);
#else
    TORCH_CHECK(false, "dlinoss_im CUDA extension was not built");
#endif
  } else {
    dlinoss_im_backward_cpu(a_diag, g_diag, step, bu, states, grad_output, grad_a, grad_g, grad_step, grad_bu);
  }
