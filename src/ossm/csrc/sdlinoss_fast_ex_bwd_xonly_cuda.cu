...
__global__ void ex_backward_xonly_kernel(
  ...
  for (int64_t tile = num_tiles - 1; tile >= 0; --tile) {
    ...
    for (int64_t k = tile_len - 1; k >= 0; --k) {
      ...
      const value_t w_prev_real =
          (t > 0) ? (x_prev.real() - x_prevprev.real()) : value_t(0);
      const value_t w_prev_imag =
          (t > 0) ? (x_prev.imag() - x_prevprev.imag()) : value_t(0);
      ...
      const value_t alpha = value_t(1) - dt * G_val;
      ...
      grad_step_local = _fma_val(grad_w_new_real, step_term_real, grad_step_local);
      grad_step_local = _fma_val(grad_w_new_imag, step_term_imag, grad_step_local);

      const value_t grad_G_term_real = -dt * w_prev_real;
      const value_t grad_G_term_imag = -dt * w_prev_imag;
      grad_G_local = _fma_val(grad_w_new_real, grad_G_term_real, grad_G_local);
      grad_G_local = _fma_val(grad_w_new_imag, grad_G_term_imag, grad_G_local);

      const value_t grad_A_term_real = -(dt2) * x_prev_real;
      const value_t grad_A_term_imag = -(dt2) * x_prev_imag;
      grad_A_local = _fma_val(grad_w_new_real, grad_A_term_real, grad_A_local);
      grad_A_local = _fma_val(grad_w_new_imag, grad_A_term_imag, grad_A_local);

      grad_x_prev_real = _fma_val(-(dt2) * A_val, grad_w_new_real, grad_x_prev_real);
      grad_x_prev_imag = _fma_val(-(dt2) * A_val, grad_w_new_imag, grad_x_prev_imag);
...
void launch_backward_kernel_xonly(const Strides3& A_stride,
                                  ...
                                  dim3 block,
                                  cudaStream_t stream) {
  ex_backward_xonly_kernel<scalar_t, VL, VB, VM, TILE><<<grid, block, 0, stream>>>(
      A,
      A_stride.sL,
      A_stride.sB,
      A_stride.sM,
...
void dispatch_backward_xonly(int vary_mask,
  ...
  const scalar_t* x_only_ptr = x_only.data_ptr<scalar_t>();
...
void run_backward_dispatch(int tile,
  ...
  switch (tile) {
    case 64:
      dispatch_backward_xonly<scalar_t, 64>(vary_mask,
                                            A_stride,
                                            G_stride,
                                            step_stride,
                                            bu_stride,
      ...
      break;
    case 256:
      dispatch_backward_xonly<scalar_t, 256>(vary_mask,
                                             A_stride,
                                             G_stride,
                                             step_stride,
                                             bu_stride,
      ...
      break;
    default:
      dispatch_backward_xonly<scalar_t, 128>(vary_mask,
                                             A_stride,
                                             G_stride,
                                             step_stride,
                                             bu_stride,
...
void sdlinoss_fast_ex_backward_xonly_cuda_complex64(int tile,
                                                    int vary_mask,
                                                    const Strides3& A_stride,
                                                    const Strides3& G_stride,
                                                    const Strides3& step_stride,
...
void sdlinoss_fast_ex_backward_xonly_cuda_complex128(int tile,
                                                     int vary_mask,
                                                     const Strides3& A_stride,
                                                     const Strides3& G_stride,
                                                     const Strides3& step_stride,
