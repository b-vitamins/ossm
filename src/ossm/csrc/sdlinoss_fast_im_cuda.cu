...
__global__ void im_build_tile_summaries_kernel(
  ...
  for (int64_t tile = 0; tile < num_tiles; ++tile) {
    ...
    for (int64_t k = 0; k < tile_len; ++k) {
      ...
      step_coeff_im<value_t>(A_val, G_val, dt, alpha, beta, gamma);
...
template <typename scalar_t>
__device__ inline FastPair<typename scalar_t::value_type> load_pair_shared(
    const typename scalar_t::value_type* m00,
...
template <typename scalar_t>
__device__ inline void store_pair_shared(typename scalar_t::value_type* m00,
                                         ...
                                         scalar_t* f1,
                                         int64_t idx,
                                         const FastPair<typename scalar_t::value_type>& value) {
  m00[idx] = value.m00;
  m01[idx] = value.m01;
  m10[idx] = value.m10;
  m11[idx] = value.m11;
...
template <typename scalar_t>
__device__ inline FastPair<typename scalar_t::value_type> identity_pair() {
  using value_t = typename scalar_t::value_type;
  FastPair<value_t> out{};
  out.m00 = value_t(1);
  out.m01 = value_t(0);
...
__global__ void im_prefix_tiles_per_series_kernel(
  ...
  if (idx >= series || num_tiles == 0) {
    return;
  }

  if (blockDim.x == 1 || num_tiles == 1) {
    FastPair<value_t> prefix = identity_pair<scalar_t>();
    for (int64_t tile = 0; tile < num_tiles; ++tile) {
  ...
  value_t* shm_m11 = shm_m10 + num_tiles;
  scalar_t* shm_f0 = reinterpret_cast<scalar_t*>(shm_m11 + num_tiles);
  scalar_t* shm_f1 = shm_f0 + num_tiles;

  for (int64_t tile = threadIdx.x; tile < num_tiles; tile += blockDim.x) {
    const int64_t offset = tile * series + idx;
    shm_m00[tile] = M00[offset];
    shm_m01[tile] = M01[offset];
    shm_m10[tile] = M10[offset];
    shm_m11[tile] = M11[offset];
    shm_f0[tile] = F0[offset];
    shm_f1[tile] = F1[offset];
  }
  __syncthreads();

  for (int64_t offset = 1; offset < num_tiles; offset <<= 1) {
    __syncthreads();
    const int64_t index = (static_cast<int64_t>(threadIdx.x) + 1) * offset * 2 - 1;
    if (index < num_tiles) {
      auto left = load_pair_shared<scalar_t>(shm_m00, shm_m01, shm_m10, shm_m11, shm_f0, shm_f1,
                                             index - offset);
      auto right = load_pair_shared<scalar_t>(shm_m00, shm_m01, shm_m10, shm_m11, shm_f0, shm_f1,
                                              index);
      auto combined = combine(right, left);
      store_pair_shared<scalar_t>(shm_m00, shm_m01, shm_m10, shm_m11, shm_f0, shm_f1, index,
                                  combined);
    }
  }
  __syncthreads();
  ...
  if (threadIdx.x == 0) {
    const auto id = identity_pair<scalar_t>();
    store_pair_shared<scalar_t>(shm_m00, shm_m01, shm_m10, shm_m11, shm_f0, shm_f1,
                                num_tiles - 1, id);
  }
  __syncthreads();
  ...
  if (num_tiles > 1) {
    max_offset = 1;
    while ((max_offset << 1) <= num_tiles) {
      max_offset <<= 1;
    }
  }

  for (int64_t offset = max_offset; offset >= 1; offset >>= 1) {
    __syncthreads();
    const int64_t index = (static_cast<int64_t>(threadIdx.x) + 1) * offset * 2 - 1;
    if (index < num_tiles) {
      auto left = load_pair_shared<scalar_t>(shm_m00, shm_m01, shm_m10, shm_m11, shm_f0, shm_f1,
  ...
  for (int64_t tile = threadIdx.x; tile < num_tiles; tile += blockDim.x) {
    const int64_t offset = tile * series + idx;
    S0_w[offset] = shm_f0[tile];
    S0_x[offset] = shm_f1[tile];
  }
}

template <typename scalar_t, bool VL, bool VB, bool VM, int TILE>
__global__ void im_expand_tiles_write_states_kernel(
    const typename scalar_t::value_type* __restrict__ A,
    int64_t AsL,
    int64_t AsB,
    int64_t AsM,
...
template <typename scalar_t, bool VL, bool VB, bool VM, int TILE>
__global__ void im_expand_tiles_write_x_kernel(
    const typename scalar_t::value_type* __restrict__ A,
    int64_t AsL,
    int64_t AsB,
    int64_t AsM,
...
void launch_build_kernel(const Strides3& A_stride,
                         ...
                         dim3 block,
                         cudaStream_t stream) {
  im_build_tile_summaries_kernel<scalar_t, VL, VB, VM, TILE>
...
void launch_expand_kernel(const Strides3& A_stride,
  ...
  im_expand_tiles_write_states_kernel<scalar_t, VL, VB, VM, TILE>
...
void launch_expand_x_kernel(const Strides3& A_stride,
  ...
  im_expand_tiles_write_x_kernel<scalar_t, VL, VB, VM, TILE>
      <<<grid, block, shared_bytes, stream>>>(A,
                                              A_stride.sL,
                                              A_stride.sB,
                                              A_stride.sM,
...
void launch_prefix_kernel(const typename scalar_t::value_type* M00,
  ...
  const size_t shared_bytes = static_cast<size_t>(num_tiles) *
                              (4 * sizeof(typename scalar_t::value_type) + 2 * sizeof(scalar_t));
  if (!fallback && shared_bytes > static_cast<size_t>(48 * 1024)) {
    fallback = true;
    block_dim = 1;
  }

  dim3 block(block_dim);
  const size_t shmem = fallback ? 0 : shared_bytes;
  im_prefix_tiles_per_series_kernel<scalar_t, TILE><<<grid, block, shmem, stream>>>(
      M00, M01, M10, M11, F0, F1, S0_w, S0_x, series, num_tiles);
}
...
template <typename scalar_t, int TILE>
void dispatch_all_x(int vary_mask,
                    const Strides3& A_stride,
                    const Strides3& G_stride,
                    const Strides3& step_stride,
                    const Strides3& bu_stride,
  ...
  scalar_t* S0_w_ptr = S0_w.data_ptr<scalar_t>();
  scalar_t* S0_x_ptr = S0_x.data_ptr<scalar_t>();
  scalar_t* x_only_ptr = x_only.data_ptr<scalar_t>();
...
template <typename scalar_t, int TILE>
void dispatch_short(int vary_mask,
                    const Strides3& A_stride,
                    const Strides3& G_stride,
                    const Strides3& step_stride,
                    const Strides3& bu_stride,
  ...
  const value_t* step_ptr = step.data_ptr<value_t>();
  const scalar_t* bu_ptr = bu.data_ptr<scalar_t>();
  const scalar_t* S0_w_ptr = S0_w.data_ptr<scalar_t>();
  const scalar_t* S0_x_ptr = S0_x.data_ptr<scalar_t>();
  scalar_t* states_ptr = states.data_ptr<scalar_t>();
...
template <typename scalar_t, int TILE>
void dispatch_short_x(int vary_mask,
                      const Strides3& A_stride,
                      const Strides3& G_stride,
                      const Strides3& step_stride,
                      const Strides3& bu_stride,
...
template <typename scalar_t>
void dispatch_forward(int tile,
                      int vary_mask,
                      const Strides3& A_stride,
                      const Strides3& G_stride,
                      const Strides3& step_stride,
  ...
  switch (tile) {
    case 64:
      dispatch_all<scalar_t, 64>(vary_mask,
                                 A_stride,
                                 G_stride,
                                 step_stride,
                                 bu_stride,
      ...
      break;
    case 256:
      dispatch_all<scalar_t, 256>(vary_mask,
                                  A_stride,
                                  G_stride,
                                  step_stride,
                                  bu_stride,
      ...
      break;
    default:
      dispatch_all<scalar_t, 128>(vary_mask,
                                  A_stride,
                                  G_stride,
                                  step_stride,
                                  bu_stride,
...
template <typename scalar_t>
void dispatch_forward_x(int tile,
                        int vary_mask,
                        const Strides3& A_stride,
                        const Strides3& G_stride,
                        const Strides3& step_stride,
  ...
  switch (tile) {
    case 64:
      dispatch_all_x<scalar_t, 64>(vary_mask,
                                   A_stride,
                                   G_stride,
                                   step_stride,
                                   bu_stride,
      ...
      break;
    case 256:
      dispatch_all_x<scalar_t, 256>(vary_mask,
                                    A_stride,
                                    G_stride,
                                    step_stride,
                                    bu_stride,
      ...
      break;
    default:
      dispatch_all_x<scalar_t, 128>(vary_mask,
                                    A_stride,
                                    G_stride,
                                    step_stride,
                                    bu_stride,
...
template <typename scalar_t>
void dispatch_short_forward_x(int tile,
                              int vary_mask,
                              const Strides3& A_stride,
                              const Strides3& G_stride,
                              const Strides3& step_stride,
  ...
  switch (tile) {
    case 64:
      dispatch_short_x<scalar_t, 64>(vary_mask,
                                     A_stride,
                                     G_stride,
                                     step_stride,
                                     bu_stride,
      ...
      break;
    case 256:
      dispatch_short_x<scalar_t, 256>(vary_mask,
                                      A_stride,
                                      G_stride,
                                      step_stride,
                                      bu_stride,
      ...
      break;
    default:
      dispatch_short_x<scalar_t, 128>(vary_mask,
                                      A_stride,
                                      G_stride,
                                      step_stride,
                                      bu_stride,
...
template <typename scalar_t>
void dispatch_short_forward(int tile,
                            int vary_mask,
                            const Strides3& A_stride,
                            const Strides3& G_stride,
                            const Strides3& step_stride,
  ...
  switch (tile) {
    case 64:
      dispatch_short<scalar_t, 64>(vary_mask,
                                   A_stride,
                                   G_stride,
                                   step_stride,
                                   bu_stride,
      ...
      break;
    case 256:
      dispatch_short<scalar_t, 256>(vary_mask,
                                    A_stride,
                                    G_stride,
                                    step_stride,
                                    bu_stride,
      ...
      break;
    default:
      dispatch_short<scalar_t, 128>(vary_mask,
                                    A_stride,
                                    G_stride,
                                    step_stride,
                                    bu_stride,
...
void sdlinoss_fast_im_forward_cuda_complex64(int tile,
                                             int vary_mask,
                                             const Strides3& A_stride,
                                             const Strides3& G_stride,
                                             const Strides3& step_stride,
                                             ...
                                             int64_t num_tiles,
                                             cudaStream_t stream) {
  dispatch_forward<c10::complex<float>>(tile,
                                        vary_mask,
                                        A_stride,
                                        G_stride,
                                        step_stride,
...
void sdlinoss_fast_im_forward_cuda_complex128(int tile,
                                             int vary_mask,
                                             const Strides3& A_stride,
                                             const Strides3& G_stride,
                                             const Strides3& step_stride,
                                              ...
                                              int64_t num_tiles,
                                              cudaStream_t stream) {
  dispatch_forward<c10::complex<double>>(tile,
                                         vary_mask,
                                         A_stride,
                                         G_stride,
                                         step_stride,
...
void sdlinoss_fast_im_forward_xonly_cuda_complex64(int tile,
                                                   int vary_mask,
                                                   const Strides3& A_stride,
                                                   const Strides3& G_stride,
                                                   const Strides3& step_stride,
                                                   ...
                                                   int64_t num_tiles,
                                                   cudaStream_t stream) {
  dispatch_forward_x<c10::complex<float>>(tile,
                                          vary_mask,
                                          A_stride,
                                          G_stride,
                                          step_stride,
...
void sdlinoss_fast_im_forward_xonly_cuda_complex128(int tile,
                                                    int vary_mask,
                                                    const Strides3& A_stride,
                                                    const Strides3& G_stride,
                                                    const Strides3& step_stride,
                                                    ...
                                                    int64_t num_tiles,
                                                    cudaStream_t stream) {
  dispatch_forward_x<c10::complex<double>>(tile,
                                           vary_mask,
                                           A_stride,
                                           G_stride,
                                           step_stride,
...
void sdlinoss_fast_im_forward_short_cuda_complex64(int tile,
                                                   int vary_mask,
                                                   const Strides3& A_stride,
                                                   const Strides3& G_stride,
                                                   const Strides3& step_stride,
                                                   ...
                                                   int64_t ssm,
                                                   cudaStream_t stream) {
  dispatch_short_forward<c10::complex<float>>(tile,
                                              vary_mask,
                                              A_stride,
                                              G_stride,
                                              step_stride,
...
void sdlinoss_fast_im_forward_short_xonly_cuda_complex64(int tile,
                                                         ...
                                                         int64_t ssm,
                                                         cudaStream_t stream) {
  dispatch_short_forward_x<c10::complex<float>>(tile,
                                                vary_mask,
                                                A_stride,
                                                G_stride,
                                                step_stride,
...
void sdlinoss_fast_im_forward_short_xonly_cuda_complex128(int tile,
                                                          ...
                                                          int64_t ssm,
                                                          cudaStream_t stream) {
  dispatch_short_forward_x<c10::complex<double>>(tile,
                                                 vary_mask,
                                                 A_stride,
                                                 G_stride,
                                                 step_stride,
...
void sdlinoss_fast_im_forward_short_cuda_complex128(int tile,
                                                    ...
                                                    int64_t ssm,
                                                    cudaStream_t stream) {
  dispatch_short_forward<c10::complex<double>>(tile,
                                               vary_mask,
                                               A_stride,
                                               G_stride,
                                               step_stride,
