...
template <typename scalar_t>
void sdlinoss_imex1_forward_cpu_kernel(
...
template <typename scalar_t>
void sdlinoss_imex1_backward_cpu_kernel(
    const typename ComplexTraits<scalar_t>::value_t* __restrict__ A,
    const typename ComplexTraits<scalar_t>::value_t* __restrict__ G,
    const typename ComplexTraits<scalar_t>::value_t* __restrict__ step,
    const scalar_t* __restrict__ bu_ptr,
...
void sdlinoss_imex1_forward_cpu(const at::Tensor& A,
                                const at::Tensor& G,
  ...
  AT_DISPATCH_COMPLEX_TYPES(bu.scalar_type(), "sdlinoss_imex1_forward_cpu", [&] {
    sdlinoss_imex1_forward_cpu_kernel<scalar_t>(A.data_ptr<typename ComplexTraits<scalar_t>::value_t>(),
...
void sdlinoss_imex1_backward_cpu(const at::Tensor& A,
                                 const at::Tensor& G,
                                 const at::Tensor& step,
                                 const at::Tensor& bu,
  ...
  AT_DISPATCH_COMPLEX_TYPES(bu.scalar_type(), "sdlinoss_imex1_backward_cpu", [&] {
    sdlinoss_imex1_backward_cpu_kernel<scalar_t>(
...
torch::Tensor sdlinoss_imex1_forward(const at::Tensor& A,
  ...
  if (bu.is_cuda()) {
#ifdef WITH_CUDA
    sdlinoss_imex1_forward_cuda(A, G, step, bu, output);
#else
    TORCH_CHECK(false, "sdlinoss_imex1 CUDA extension was not built");
#endif
  } else {
    sdlinoss_imex1_forward_cpu(A, G, step, bu, output);
  }
...
std::vector<at::Tensor> sdlinoss_imex1_backward(const at::Tensor& A,
  ...
  if (bu.is_cuda()) {
#ifdef WITH_CUDA
    sdlinoss_imex1_backward_cuda(A, G, step, bu, states, grad_output, grad_A, grad_G, grad_step, grad_bu);
#else
    TORCH_CHECK(false, "sdlinoss_imex1 CUDA extension was not built");
#endif
  } else {
    sdlinoss_imex1_backward_cpu(A, G, step, bu, states, grad_output, grad_A, grad_G, grad_step, grad_bu);
  }
