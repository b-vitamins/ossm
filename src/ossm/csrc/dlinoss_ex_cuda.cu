...
template <typename scalar_t>
__global__ void dlinoss_ex_backward_tile_kernel(const typename scalar_t::value_type* __restrict__ a_diag,
                                                const typename scalar_t::value_type* __restrict__ g_diag,
                                                const typename scalar_t::value_type* __restrict__ step,
                                                const scalar_t* __restrict__ bu_ptr,
...
template <typename scalar_t>
__global__ void dlinoss_ex_backward_reduce_kernel(
...
void dlinoss_ex_backward_cuda(const at::Tensor& a_diag,
  ...
  AT_DISPATCH_COMPLEX_TYPES(bu.scalar_type(), "dlinoss_ex_backward_cuda", [&] {
    ...
    dlinoss_ex_backward_tile_kernel<scalar_t><<<blocks, threads, shared_mem_size, stream>>>(
    ...
    dlinoss_ex_backward_reduce_kernel<scalar_t><<<reduce_blocks, reduce_threads, 0, stream>>>(
        grad_a_partial.data_ptr<value_t>(),
        grad_g_partial.data_ptr<value_t>(),
        grad_step_partial.data_ptr<value_t>(),
