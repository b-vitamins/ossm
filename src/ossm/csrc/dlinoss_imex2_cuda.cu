...
__global__ void dlinoss_imex2_forward_kernel(const typename scalar_t::value_type* __restrict__ a_diag,
  ...
  for (int64_t idx = blockIdx.x * blockDim.x + threadIdx.x; idx < series; idx += blockDim.x * gridDim.x) {
    ...
    for (int64_t t = 0; t < length; ++t) {
      ...
      const value_t new1_real = state0_real * m21 + state1_real * m22 + bu_real * sigma_sq;
      const value_t new1_imag = state0_imag * m21 + state1_imag * m22 + bu_imag * sigma_sq;
...
__global__ void dlinoss_imex2_backward_kernel(const typename scalar_t::value_type* __restrict__ a_diag,
  ...
  for (int64_t idx = blockIdx.x * blockDim.x + threadIdx.x; idx < series; idx += blockDim.x * gridDim.x) {
    ...
    const value_t d_sigma_prev0 = value_t(1) - value_t(2) * sigma * gamma;
    ...
    for (int64_t t = length - 1; t >= 0; --t) {
      ...
      grad_alpha_local += grad_new0_real * (-sigma * prev1_real) + grad_new0_imag * (-sigma * prev1_imag);
      grad_alpha_local += grad_new1_real * (-sigma_sq * prev1_real) + grad_new1_imag * (-sigma_sq * prev1_imag);

      grad_gamma_local += grad_new0_real * (-sigma * prev0_real) + grad_new0_imag * (-sigma * prev0_imag);
      grad_gamma_local += grad_new1_real * (-sigma_sq * prev0_real) + grad_new1_imag * (-sigma_sq * prev0_imag);

      const value_t sigma_term0_real = (-gamma) * prev0_real;
      const value_t sigma_term0_imag = (-gamma) * prev0_imag;
      const value_t sigma_term1_real = (-alpha) * prev1_real;
      const value_t sigma_term1_imag = (-alpha) * prev1_imag;

      grad_sigma_local += grad_new0_real * (sigma_term0_real + sigma_term1_real + bu_real);
      grad_sigma_local += grad_new0_imag * (sigma_term0_imag + sigma_term1_imag + bu_imag);

      grad_sigma_local += grad_new1_real * (d_sigma_prev0 * prev0_real + d_sigma_prev1 * prev1_real + d_sigma_bu * bu_real);
      grad_sigma_local += grad_new1_imag * (d_sigma_prev0 * prev0_imag + d_sigma_prev1 * prev1_imag + d_sigma_bu * bu_imag);
      ...
      const value_t grad_prev0_real = grad_new0_real * m11 + grad_new1_real * m21;
      const value_t grad_prev0_imag = grad_new0_imag * m11 + grad_new1_imag * m21;
      const value_t grad_prev1_real = grad_new0_real * m12 + grad_new1_real * m22;
      const value_t grad_prev1_imag = grad_new0_imag * m12 + grad_new1_imag * m22;
...
void dlinoss_imex2_forward_cuda(const at::Tensor& a_diag,
  ...
  AT_DISPATCH_COMPLEX_TYPES(bu.scalar_type(), "dlinoss_imex2_forward_cuda", [&] {
    dlinoss_imex2_forward_kernel<scalar_t><<<blocks, threads, 0, at::cuda::getCurrentCUDAStream()>>>(
