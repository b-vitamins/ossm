...
void sdlinoss_fast_im_backward_xonly_cuda_complex64(int tile,
                                                    int vary_mask,
                                                    const Strides3& A_stride,
                                                    const Strides3& G_stride,
                                                    const Strides3& step_stride,
...
void sdlinoss_fast_im_backward_xonly_cuda_complex128(int tile,
                                                     int vary_mask,
                                                     const Strides3& A_stride,
                                                     const Strides3& G_stride,
                                                     const Strides3& step_stride,
...
at::Tensor sdlinoss_fast_im_forward(const at::Tensor& A,
                                    ...
                                    const at::Tensor& bu) {
#ifndef WITH_CUDA
  TORCH_CHECK(false, "sdlinoss_fast_im_forward requires CUDA support.");
#else
  ...
  if (num_tiles == 1) {
    auto S0_w = at::zeros({1, series}, options_complex);
    auto S0_x = at::zeros({1, series}, options_complex);

    AT_DISPATCH_COMPLEX_TYPES(bu.scalar_type(), "sdlinoss_fast_im_forward_short", [&] {
      if constexpr (std::is_same_v<scalar_t, c10::complex<float>>) {
        sdlinoss_fast_im_forward_short_cuda_complex64(tile,
                                                      vary_mask,
                                                      A_stride,
                                                      G_stride,
                                                      step_stride,
        ...
        sdlinoss_fast_im_forward_short_cuda_complex128(tile,
                                                       vary_mask,
                                                       A_stride,
                                                       G_stride,
                                                       step_stride,
  ...
  AT_DISPATCH_COMPLEX_TYPES(bu.scalar_type(), "sdlinoss_fast_im_forward", [&] {
    if constexpr (std::is_same_v<scalar_t, c10::complex<float>>) {
      sdlinoss_fast_im_forward_cuda_complex64(tile,
                                              vary_mask,
                                              A_stride,
                                              G_stride,
                                              step_stride,
      ...
      sdlinoss_fast_im_forward_cuda_complex128(tile,
                                               vary_mask,
                                               A_stride,
                                               G_stride,
                                               step_stride,
...
at::Tensor sdlinoss_fast_im_forward_xonly(const at::Tensor& A,
                                          ...
                                          const at::Tensor& bu) {
#ifndef WITH_CUDA
  TORCH_CHECK(false, "sdlinoss_fast_im_forward_xonly requires CUDA support.");
#else
  TORCH_CHECK(A.is_cuda(), "A must be a CUDA tensor.");
  TORCH_CHECK(G.is_cuda(), "G must be a CUDA tensor.");
  TORCH_CHECK(step.is_cuda(), "step must be a CUDA tensor.");
  ...
  if (num_tiles == 1) {
    ...
    AT_DISPATCH_COMPLEX_TYPES(bu.scalar_type(), "sdlinoss_fast_im_forward_short_xonly", [&] {
      if constexpr (std::is_same_v<scalar_t, c10::complex<float>>) {
        sdlinoss_fast_im_forward_short_xonly_cuda_complex64(tile,
                                                            vary_mask,
                                                            A_stride,
                                                            G_stride,
                                                            step_stride,
        ...
        sdlinoss_fast_im_forward_short_xonly_cuda_complex128(tile,
                                                             vary_mask,
                                                             A_stride,
                                                             G_stride,
                                                             step_stride,
  ...
  AT_DISPATCH_COMPLEX_TYPES(bu.scalar_type(), "sdlinoss_fast_im_forward_xonly", [&] {
    if constexpr (std::is_same_v<scalar_t, c10::complex<float>>) {
      sdlinoss_fast_im_forward_xonly_cuda_complex64(tile,
                                                    vary_mask,
                                                    A_stride,
                                                    G_stride,
                                                    step_stride,
      ...
      sdlinoss_fast_im_forward_xonly_cuda_complex128(tile,
                                                     vary_mask,
                                                     A_stride,
                                                     G_stride,
                                                     step_stride,
...
std::tuple<at::Tensor, at::Tensor, at::Tensor, at::Tensor> sdlinoss_fast_im_backward(
    ...
    const at::Tensor& grad_out) {
#ifndef WITH_CUDA
  TORCH_CHECK(false, "sdlinoss_fast_im_backward requires CUDA support.");
#else
  TORCH_CHECK(A.is_cuda(), "A must be a CUDA tensor.");
  TORCH_CHECK(G.is_cuda(), "G must be a CUDA tensor.");
  TORCH_CHECK(step.is_cuda(), "step must be a CUDA tensor.");
  ...
  AT_DISPATCH_COMPLEX_TYPES(bu.scalar_type(), "sdlinoss_fast_im_backward", [&] {
    if constexpr (std::is_same_v<scalar_t, c10::complex<float>>) {
      sdlinoss_fast_im_backward_cuda_complex64(tile,
                                               vary_mask,
                                               strides.A,
                                               strides.G,
                                               strides.step,
      ...
      sdlinoss_fast_im_backward_cuda_complex128(tile,
                                                vary_mask,
                                                strides.A,
                                                strides.G,
                                                strides.step,
...
std::tuple<at::Tensor, at::Tensor, at::Tensor, at::Tensor> sdlinoss_fast_im_backward_xonly(
    ...
    const at::Tensor& grad_out) {
#ifndef WITH_CUDA
  TORCH_CHECK(false, "sdlinoss_fast_im_backward_xonly requires CUDA support.");
#else
  TORCH_CHECK(A.is_cuda(), "A must be a CUDA tensor.");
  TORCH_CHECK(G.is_cuda(), "G must be a CUDA tensor.");
  TORCH_CHECK(step.is_cuda(), "step must be a CUDA tensor.");
  ...
  TORCH_CHECK(real_dtype == G.scalar_type() && real_dtype == step.scalar_type(),
              "A, G, and step must share the same real dtype.");
  TORCH_CHECK(bu.scalar_type() == x_only.scalar_type(),
              "x_only dtype must match bu dtype.");
  TORCH_CHECK(grad_out.scalar_type() == bu.scalar_type(),
              "grad_out dtype must match bu dtype.");
  TORCH_CHECK((bu.scalar_type() == at::kComplexFloat && real_dtype == at::kFloat) ||
                  (bu.scalar_type() == at::kComplexDouble && real_dtype == at::kDouble),
  ...
  AT_DISPATCH_COMPLEX_TYPES(bu.scalar_type(), "sdlinoss_fast_im_backward_xonly", [&] {
    if constexpr (std::is_same_v<scalar_t, c10::complex<float>>) {
      sdlinoss_fast_im_backward_xonly_cuda_complex64(tile,
                                                     vary_mask,
                                                     strides.A,
                                                     strides.G,
                                                     strides.step,
      ...
      sdlinoss_fast_im_backward_xonly_cuda_complex128(tile,
                                                      vary_mask,
                                                      strides.A,
                                                      strides.G,
                                                      strides.step,
