...
__global__ void imex1_backward_xonly_kernel(
  ...
  for (int64_t tile = num_tiles - 1; tile >= 0; --tile) {
    ...
    for (int64_t k = tile_len - 1; k >= 0; --k) {
      ...
      const value_t w_prev_real = (t > 0) ? (x_prev.real() - x_prevprev.real()) : value_t(0);
      const value_t w_prev_imag = (t > 0) ? (x_prev.imag() - x_prevprev.imag()) : value_t(0);
      ...
      const value_t S_raw = value_t(1) + dt * G_val;
...
void launch_backward_kernel_xonly(const Strides3& A_stride,
                                  ...
                                  dim3 block,
                                  cudaStream_t stream) {
  imex1_backward_xonly_kernel<scalar_t, VL, VB, VM, TILE><<<grid, block, 0, stream>>>(
      A,
      A_stride.sL,
      A_stride.sB,
      A_stride.sM,
...
void sdlinoss_fast_imex1_backward_xonly_cuda_complex64(int tile,
                                                       int vary_mask,
                                                       const Strides3& A_stride,
                                                       const Strides3& G_stride,
                                                       const Strides3& step_stride,
...
void sdlinoss_fast_imex1_backward_xonly_cuda_complex128(int tile,
                                                        int vary_mask,
                                                        const Strides3& A_stride,
                                                        const Strides3& G_stride,
                                                        const Strides3& step_stride,
