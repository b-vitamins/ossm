...
constexpr int kThreads = kTile;  // 1 thread per step
...
__global__ void sdlinoss_imex1_forward_tile_kernel(
  ...
  if (t < length) {
    const int b = series / ssm;
    const int m = series % ssm;
    const int64_t off = static_cast<int64_t>(t) * series_n + series;
...
#pragma unroll
  ...
  if (t < length) {
    ...
    const int64_t out_off = static_cast<int64_t>(t) * step_stride + series * 2;
  ...
  if (t_in_tile == kTile - 1) {
    ...
    const int64_t idx_tile = static_cast<int64_t>(series) * ntile + tile_id;
    int last = min(kTile, static_cast<int>(length - t0)) - 1;
    if (last < 0) {
      last = 0;
    }
