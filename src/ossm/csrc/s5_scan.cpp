...
at::Tensor s5_scan_cpu(const at::Tensor& lambda_real,
...
// to match the definition in s5_scan_cuda.cu and avoid undefined symbols.
...
at::Tensor s5_scan(const at::Tensor& lambda_real,
  ...
  if (b_seq.is_cuda()) {
#ifdef WITH_CUDA
    return s5_scan_cuda(lambda_real, lambda_imag, b_seq);
#else
    TORCH_CHECK(false, "s5_scan CUDA extension was not built");
#endif
  }
