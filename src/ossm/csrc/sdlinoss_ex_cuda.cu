...
struct ExTransitionBuilder {
  ...
  __device__ static Transition2x2<value_t> build(value_t a, value_t g, value_t step, scalar_t bu) {
    ...
    const value_t m00 = value_t(1) - dt * g;
    const value_t m01 = -a * dt;
    out.m00 = m00;
    out.m01 = m01;
    out.m10 = dt * m00;
    ...
    const value_t f0_scale = dt;
    const value_t f1_scale = dt * dt;
...
void sdlinoss_ex_forward_cuda(const at::Tensor& A,
  ...
  AT_DISPATCH_COMPLEX_TYPES(bu.scalar_type(), "sdlinoss_ex_forward_cuda", [&] {
    ...
    sdlinoss_forward_tile_kernel<scalar_t, ExTransitionBuilder<scalar_t>, kSdlinossTileSteps>
