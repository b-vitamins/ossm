...
void sdlinoss_fast_imex1_forward_cuda_complex64(int tile,
                                                int vary_mask,
...
void sdlinoss_fast_imex1_forward_cuda_complex128(int tile,
                                                 int vary_mask,
                                                 const Strides3& A_stride,
                                                 const Strides3& G_stride,
                                                 const Strides3& step_stride,
...
void sdlinoss_fast_imex1_forward_xonly_cuda_complex64(int tile,
                                                      int vary_mask,
                                                      const Strides3& A_stride,
                                                      const Strides3& G_stride,
                                                      const Strides3& step_stride,
...
void sdlinoss_fast_imex1_forward_xonly_cuda_complex128(int tile,
                                                       int vary_mask,
                                                       const Strides3& A_stride,
                                                       const Strides3& G_stride,
                                                       const Strides3& step_stride,
...
void sdlinoss_fast_imex1_backward_cuda_complex64(int tile,
                                                 int vary_mask,
                                                 const Strides3& A_stride,
                                                 const Strides3& G_stride,
                                                 const Strides3& step_stride,
...
void sdlinoss_fast_imex1_backward_cuda_complex128(int tile,
                                                  int vary_mask,
                                                  const Strides3& A_stride,
                                                  const Strides3& G_stride,
                                                  const Strides3& step_stride,
...
at::Tensor sdlinoss_fast_imex1_forward(const at::Tensor& A,
                                       ...
                                       const at::Tensor& bu) {
#ifndef WITH_CUDA
  TORCH_CHECK(false, "sdlinoss_fast_imex1_forward requires CUDA support.");
#else
  ...
  AT_DISPATCH_COMPLEX_TYPES(bu.scalar_type(), "sdlinoss_fast_imex1_forward", [&] {
    if constexpr (std::is_same_v<scalar_t, c10::complex<float>>) {
      sdlinoss_fast_imex1_forward_cuda_complex64(tile,
                                                 vary_mask,
                                                 A_stride,
                                                 G_stride,
                                                 step_stride,
      ...
      sdlinoss_fast_imex1_forward_cuda_complex128(tile,
                                                  vary_mask,
                                                  A_stride,
                                                  G_stride,
                                                  step_stride,
...
at::Tensor sdlinoss_fast_imex1_forward_xonly(const at::Tensor& A,
                                             ...
                                             const at::Tensor& bu) {
#ifndef WITH_CUDA
  TORCH_CHECK(false, "sdlinoss_fast_imex1_forward_xonly requires CUDA support.");
#else
  TORCH_CHECK(A.is_cuda(), "A must be a CUDA tensor.");
  TORCH_CHECK(G.is_cuda(), "G must be a CUDA tensor.");
  TORCH_CHECK(step.is_cuda(), "step must be a CUDA tensor.");
  ...
  AT_DISPATCH_COMPLEX_TYPES(bu.scalar_type(), "sdlinoss_fast_imex1_forward_xonly", [&] {
    if constexpr (std::is_same_v<scalar_t, c10::complex<float>>) {
      sdlinoss_fast_imex1_forward_xonly_cuda_complex64(tile,
                                                       vary_mask,
                                                       A_stride,
                                                       G_stride,
                                                       step_stride,
      ...
      sdlinoss_fast_imex1_forward_xonly_cuda_complex128(tile,
                                                        vary_mask,
                                                        A_stride,
                                                        G_stride,
                                                        step_stride,
...
std::tuple<at::Tensor, at::Tensor, at::Tensor, at::Tensor> sdlinoss_fast_imex1_backward(
    ...
    const at::Tensor& grad_out) {
#ifndef WITH_CUDA
  TORCH_CHECK(false, "sdlinoss_fast_imex1_backward requires CUDA support.");
#else
  TORCH_CHECK(A.is_cuda(), "A must be a CUDA tensor.");
  TORCH_CHECK(G.is_cuda(), "G must be a CUDA tensor.");
  TORCH_CHECK(step.is_cuda(), "step must be a CUDA tensor.");
  ...
  AT_DISPATCH_COMPLEX_TYPES(bu.scalar_type(), "sdlinoss_fast_imex1_backward", [&] {
    if constexpr (std::is_same_v<scalar_t, c10::complex<float>>) {
      sdlinoss_fast_imex1_backward_cuda_complex64(tile,
                                                  vary_mask,
                                                  strides.A,
                                                  strides.G,
                                                  strides.step,
      ...
      sdlinoss_fast_imex1_backward_cuda_complex128(tile,
                                                   vary_mask,
                                                   strides.A,
                                                   strides.G,
                                                   strides.step,
...
std::tuple<at::Tensor, at::Tensor, at::Tensor, at::Tensor> sdlinoss_fast_imex1_backward_xonly(
    ...
    const at::Tensor& grad_out) {
#ifndef WITH_CUDA
  TORCH_CHECK(false, "sdlinoss_fast_imex1_backward_xonly requires CUDA support.");
#else
  TORCH_CHECK(A.is_cuda(), "A must be a CUDA tensor.");
  TORCH_CHECK(G.is_cuda(), "G must be a CUDA tensor.");
  TORCH_CHECK(step.is_cuda(), "step must be a CUDA tensor.");
  ...
  AT_DISPATCH_COMPLEX_TYPES(bu.scalar_type(), "sdlinoss_fast_imex1_backward_xonly", [&] {
    if constexpr (std::is_same_v<scalar_t, c10::complex<float>>) {
      sdlinoss_fast_imex1_backward_xonly_cuda_complex64(tile,
                                                        vary_mask,
                                                        strides.A,
                                                        strides.G,
                                                        strides.step,
      ...
      sdlinoss_fast_imex1_backward_xonly_cuda_complex128(tile,
                                                         vary_mask,
                                                         strides.A,
                                                         strides.G,
                                                         strides.step,
