...
__global__ void imex2_backward_xonly_kernel(
    ...
    int64_t BusB,
    int64_t BusM,
    const scalar_t* __restrict__ x_only,
  ...
  for (int64_t tile = num_tiles - 1; tile >= 0; --tile) {
    ...
    for (int64_t k = tile_len - 1; k >= 0; --k) {
      ...
      const scalar_t x_new = x_only[offset_series];
      const scalar_t x_prev = (t > 0) ? x_only[offset_series - series] : scalar_t(0);
      const scalar_t x_prevprev = (t > 1) ? x_only[offset_series - 2 * series] : scalar_t(0);

      const value_t x_new_real = x_new.real();
      const value_t x_new_imag = x_new.imag();
      const value_t x_prev_real = x_prev.real();
      const value_t x_prev_imag = x_prev.imag();
      ...
      const value_t w_prev_imag =
          (t > 0) ? (x_prev_imag - x_prevprev_imag) : value_t(0);
      ...
      const value_t S_raw = value_t(1) + dt2 * A_val;
      ...
      const value_t alpha_raw = value_t(1) - dt * G_val;
      ...
      const value_t tmpw_real =
          _fma_val(dt2, comb_real, alpha_raw * w_prev_real);
      const value_t tmpw_imag =
          _fma_val(dt2, comb_imag, alpha_raw * w_prev_imag);
      ...
      grad_step_local =
          _fma_val(grad_S_raw, value_t(2) * dt * A_val, grad_step_local);
      grad_A_local = _fma_val(grad_S_raw, dt2, grad_A_local);

      const value_t step_term_real =
          _fma_val(value_t(-G_val), w_prev_real, value_t(2) * dt * comb_real);
      const value_t step_term_imag =
          _fma_val(value_t(-G_val), w_prev_imag, value_t(2) * dt * comb_imag);
      grad_step_local = _fma_val(grad_tmpw_real, step_term_real, grad_step_local);
      grad_step_local = _fma_val(grad_tmpw_imag, step_term_imag, grad_step_local);
      ...
      const value_t grad_G_term_real = -(dt) * w_prev_real;
      const value_t grad_G_term_imag = -(dt) * w_prev_imag;
      grad_G_local = _fma_val(grad_tmpw_real, grad_G_term_real, grad_G_local);
      grad_G_local = _fma_val(grad_tmpw_imag, grad_G_term_imag, grad_G_local);
      ...
      const value_t grad_w_prev_real = grad_tmpw_real * alpha_raw;
      const value_t grad_w_prev_imag = grad_tmpw_imag * alpha_raw;
...
template <typename scalar_t, bool VL, bool VB, bool VM, int TILE>
void launch_backward_kernel_xonly(const Strides3& A_stride,
                                  ...
                                  dim3 block,
                                  cudaStream_t stream) {
  imex2_backward_xonly_kernel<scalar_t, VL, VB, VM, TILE><<<grid, block, 0, stream>>>(
      A,
      A_stride.sL,
      A_stride.sB,
      A_stride.sM,
...
void dispatch_backward_xonly(int vary_mask,
  ...
  const scalar_t* x_ptr = x_only.data_ptr<scalar_t>();
  const scalar_t* grad_ptr = grad_out.data_ptr<scalar_t>();
  scalar_t* grad_bu_ptr = grad_bu.data_ptr<scalar_t>();

  auto grad_A_info = make_grad_strided3<value_t>(grad_A, length, batch, ssm);
  auto grad_G_info = make_grad_strided3<value_t>(grad_G, length, batch, ssm);
  auto grad_step_info = make_grad_strided3<value_t>(grad_step, length, batch, ssm);

  switch (vary_mask) {
    case 0:
      launch_backward_kernel_xonly<scalar_t, false, false, false, TILE>(A_stride,
                                                                        G_stride,
                                                                        step_stride,
                                                                        bu_stride,
                                                                        A_ptr,
      ...
      break;
    case 1:
      launch_backward_kernel_xonly<scalar_t, true, false, false, TILE>(A_stride,
                                                                       G_stride,
                                                                       step_stride,
                                                                       bu_stride,
                                                                       A_ptr,
      ...
      break;
    case 2:
      launch_backward_kernel_xonly<scalar_t, false, true, false, TILE>(A_stride,
                                                                       G_stride,
                                                                       step_stride,
                                                                       bu_stride,
                                                                       A_ptr,
      ...
      break;
    case 3:
      launch_backward_kernel_xonly<scalar_t, true, true, false, TILE>(A_stride,
                                                                      G_stride,
                                                                      step_stride,
                                                                      bu_stride,
                                                                      A_ptr,
      ...
      break;
    case 4:
      launch_backward_kernel_xonly<scalar_t, false, false, true, TILE>(A_stride,
                                                                       G_stride,
                                                                       step_stride,
                                                                       bu_stride,
                                                                       A_ptr,
      ...
      break;
    case 5:
      launch_backward_kernel_xonly<scalar_t, true, false, true, TILE>(A_stride,
                                                                      G_stride,
                                                                      step_stride,
                                                                      bu_stride,
                                                                      A_ptr,
      ...
      break;
    case 6:
      launch_backward_kernel_xonly<scalar_t, false, true, true, TILE>(A_stride,
                                                                      G_stride,
                                                                      step_stride,
                                                                      bu_stride,
                                                                      A_ptr,
      ...
      break;
    default:
      launch_backward_kernel_xonly<scalar_t, true, true, true, TILE>(A_stride,
                                                                     G_stride,
                                                                     step_stride,
                                                                     bu_stride,
                                                                     A_ptr,
...
void sdlinoss_fast_imex2_backward_xonly_cuda_complex64(int tile,
  ...
  switch (tile) {
    case 64:
      dispatch_backward_xonly<c10::complex<float>, 64>(vary_mask,
                                                       A_stride,
                                                       G_stride,
                                                       step_stride,
                                                       bu_stride,
      ...
      break;
    case 256:
      dispatch_backward_xonly<c10::complex<float>, 256>(vary_mask,
                                                        A_stride,
                                                        G_stride,
                                                        step_stride,
                                                        bu_stride,
      ...
      break;
    default:
      dispatch_backward_xonly<c10::complex<float>, 128>(vary_mask,
                                                        A_stride,
                                                        G_stride,
                                                        step_stride,
                                                        bu_stride,
...
void sdlinoss_fast_imex2_backward_xonly_cuda_complex128(int tile,
  ...
  switch (tile) {
    case 64:
      dispatch_backward_xonly<c10::complex<double>, 64>(vary_mask,
                                                        A_stride,
                                                        G_stride,
                                                        step_stride,
                                                        bu_stride,
      ...
      break;
    case 256:
      dispatch_backward_xonly<c10::complex<double>, 256>(vary_mask,
                                                         A_stride,
                                                         G_stride,
                                                         step_stride,
                                                         bu_stride,
      ...
      break;
    default:
      dispatch_backward_xonly<c10::complex<double>, 128>(vary_mask,
                                                         A_stride,
                                                         G_stride,
                                                         step_stride,
                                                         bu_stride,
