#!/usr/bin/env python
"""Benchmark LRU PyTorch kernels against reference JAX implementation."""
...
from models.LRU import LRULayer as JaxLRULayer  # noqa: E402

from ossm.models import lru as torch_lru  # noqa: E402
from ossm.models import _lru_scan  # noqa: E402
...
def _disable_kernel(flag: bool):
    if not flag:
        yield
        return
    old_kernels = getattr(_lru_scan, "_kernels", None)
    _lru_scan._kernels = None  # type: ignore[attr-defined]
    try:
        yield
    finally:
        _lru_scan._kernels = old_kernels  # type: ignore[attr-defined]
...
def _build_layers(ssm_size: int, hidden_dim: int, *, r_min: float, r_max: float, max_phase: float):
    ...
    layer = torch_lru.LRULayer(ssm_size=ssm_size, hidden_dim=hidden_dim, r_min=r_min, r_max=r_max, max_phase=max_phase).eval()

    params = {
        "nu_log": _to_numpy(layer.nu_log),
        "theta_log": _to_numpy(layer.theta_log),
        "B_re": _to_numpy(layer.B[..., 0]),
        "B_im": _to_numpy(layer.B[..., 1]),
        "C_re": _to_numpy(layer.C[..., 0]),
        "C_im": _to_numpy(layer.C[..., 1]),
        "D": _to_numpy(layer.D),
        "gamma_log": _to_numpy(layer.gamma_log),
    }

    key = jr.PRNGKey(0)
    jax_layer = JaxLRULayer(ssm_size, hidden_dim, r_min=r_min, r_max=r_max, max_phase=max_phase, key=key)
...
def _measure_jax(jax_layer: JaxLRULayer, inputs: jnp.ndarray) -> tuple[float, jnp.ndarray]:
...
def main() -> None:
    if not _lru_scan.is_available():
        error = _lru_scan.extension_error()
        details = f"\nLast extension error: {error}" if error is not None else ""
        raise RuntimeError(
            "LRU custom kernels are unavailable. Install OSSM with the `linoss` extra "
    ...
    layer_double = torch_lru.LRULayer(ssm_size=ssm_size, hidden_dim=hidden_dim, r_min=r_min, r_max=r_max, max_phase=max_phase).double().eval()
