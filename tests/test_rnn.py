...
def test_linear_rnn_extension_preserves_gradients() -> None:
    torch.manual_seed(123)
    batch, length, input_dim, hidden_dim = 4, 7, 5, 3
    inputs = torch.randn(batch, length, input_dim, requires_grad=True)

    cell = LinearRNNCell(input_dim, hidden_dim)
    layer = RNNLayer(cell)

    features, final_state = layer(inputs)
    loss = features.sum() + final_state.sum()
    loss.backward()

    assert inputs.grad is not None
    assert torch.count_nonzero(inputs.grad) > 0
    for param in cell.linear.parameters():
