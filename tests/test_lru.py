...
def test_lru_gradients_flow_through_extension() -> None:
    torch.manual_seed(7)
    layer = LRULayer(ssm_size=6, hidden_dim=4)
    inputs = torch.randn(3, 10, 4, requires_grad=True)

    loss = layer(inputs).sum()
    loss.backward()

    assert inputs.grad is not None and inputs.grad.abs().sum() > 0
    assert layer.nu_log.grad is not None and layer.nu_log.grad.abs().sum() > 0
