...
def test_fast_imex2_xonly_matches_states(
    length: int, batch: int, ssm: int, dtype: torch.dtype, shape_tag: str
) -> None:
    if not torch.cuda.is_available():
        pytest.skip("CUDA is required for fast selective D-LinOSS kernels")

    if not has_fast_kernels("imex2"):
        pytest.skip("Fast IMEX2 kernels are unavailable in this build")
    ...
    x_only = kernels.sdlinoss_fast_imex2_forward_xonly(A_param, G_param, step_param, bu)
    states = kernels.sdlinoss_fast_imex2_forward(A_param, G_param, step_param, bu)
...
@pytest.mark.parametrize("dtype", [torch.complex64, torch.complex128])
@pytest.mark.parametrize("shape_tag", ["M", "LM", "BM", "LBM"])
def test_fast_imex2_backward_xonly_matches_states(
    length: int, batch: int, ssm: int, dtype: torch.dtype, shape_tag: str
) -> None:
    if not torch.cuda.is_available():
        pytest.skip("CUDA is required for fast selective D-LinOSS kernels")
    ...
    gA_x, gG_x, gStep_x, gBu_x = kernels.sdlinoss_fast_imex2_backward_xonly(
        A_param, G_param, step_param, bu, x_only, grad_out
    )
    gA, gG, gStep, gBu = kernels.sdlinoss_fast_imex2_backward(
...
@pytest.mark.cuda
def test_fast_imex2_autograd_invokes_xonly_kernels(monkeypatch: pytest.MonkeyPatch) -> None:
    if not torch.cuda.is_available():
        pytest.skip("CUDA is required for fast selective D-LinOSS kernels")
    ...
    orig_forward = kernels.sdlinoss_fast_imex2_forward_xonly
    orig_backward = kernels.sdlinoss_fast_imex2_backward_xonly
    ...
    monkeypatch.setattr(kernels, "sdlinoss_fast_imex2_forward_xonly", forward_wrapper)
    monkeypatch.setattr(kernels, "sdlinoss_fast_imex2_backward_xonly", backward_wrapper)
    monkeypatch.setattr(fast, "X_ONLY", True)

    output = fast.SdlinossImex2FastFn.apply(A, G, step, bu)
