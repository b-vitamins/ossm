...
def test_s5_gradients_flow_through_extension() -> None:
    torch.manual_seed(6)
    layer = S5Layer(ssm_size=6, hidden_dim=4, blocks=1)
    inputs = torch.randn(3, 9, 4, requires_grad=True)

    loss = layer(inputs).sum()
    loss.backward()

    assert inputs.grad is not None and inputs.grad.abs().sum() > 0
    assert layer.lambda_real.grad is not None and layer.lambda_real.grad.abs().sum() > 0
